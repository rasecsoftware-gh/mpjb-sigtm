<script>
	ctl.plani_traba_window = function(record) {
		var w_config = {
			id:'ctl_plani_traba_window',
			...
			items:[{
				...
				items: [{
					xtype: 'grid',
					title: 'Ingresos',
					id: 'ctl_plani_traba_ingreso_grid',
					region: 'west',
					width: 300,
					split: true,
					emptyText: 'No tiene ingresos registrados',
					store: ctl.plani_traba_ingreso_store,
					...
					xtype: 'grid',
					title: 'Descuentos',
					id: 'ctl_plani_traba_descuento_grid',
					region: 'center',
					emptyText: 'No tiene descuentos registrados',
					store: ctl.plani_traba_descuento_store,
					sortableColumns: false,
					enableColumnHide: false,
					features: [{
				    	ftype: 'summary'
					...
					xtype: 'grid',
					title: 'Aportes',
					id: 'ctl_plani_traba_aporte_grid',
					region: 'east',
					width: 300,
					split: true,
					emptyText: 'No tiene aportes registrados',
					store: ctl.plani_traba_aporte_store,
					sortableColumns: false,
					enableColumnHide: false,
					features: [{
				    	ftype: 'summary'
			...
			buttons:[{
				text: 'Salir', handler: function() {
					win = Ext.getCmp('ctl_plani_traba_window');
					win.close();
				}
			}],
			listeners: {
				show: function () {
					...
					Ext.getCmp('ctl_plani_traba_form').loadRecord(record);

					ctl.plani_traba_ingreso_store.reload({
						params: {
					...
					ctl.plani_traba_descuento_store.reload({
						params: {
							plani_anio: record.get('plani_anio'),
							plani_cod: record.get('plani_cod'),
		    				traba_cod: record.get('traba_cod')
					...
					ctl.plani_traba_aporte_store.reload({
						params: {
							plani_anio: record.get('plani_anio'),
							plani_cod: record.get('plani_cod'),
		    				traba_cod: record.get('traba_cod')
