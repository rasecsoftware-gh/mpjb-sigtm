...
?>
		...
		<tr>
			<td><label for="tipo_permiso_id_field">Tipo de Permiso:</label></td>
			<td>
			    <select id="tipo_permiso_id_field" name="tipo_permiso_id" style="width: 300px;">
<?php 
	foreach ($tipo_permiso_list as $r):
		$selected = ( $r->tipo_permiso_id == $p_tipo_permiso_id )?'selected="selected"':'';
?>
			      <option value="<?=$r->tipo_permiso_id?>" <?=$selected?> ><?=$r->tipo_permiso_desc?></option>
<?php
	endforeach;
?>
			    </select>
				<script>
				$("#tipo_permiso_id_field").selectmenu({
					width: 400
				});
				</script>
			</td>
		...
		<tr>
			...
			<td>
			    <input id="resolucion_field" type="text" name="resolucion" value="<?=$p_resolucion?>" style="width: 150px;">
			</td>
		</tr>
		<tr>
...
?>
	...
	<div style="width: 100%;">
		...
		<thead>
			<tr>
				...
				<th >APELLIDOS</th>
				<th >FECHA</th>
				<th title="Tipo de Permiso">TIPO DE PERMISO</th>
				<th >FECHA INICIO</th>
				<th >FECHA FIN</th>
				<th >RESOLUCION</th>
				<th >RUTA</th>
	...
	?>
			<tr>
				<td title="<?=$r->psp_id?>"><?=$nr?></td>
				<td><?=$r->psp_anio?></td>
				<td>
					<a class="list-row" href="#<?=$r->psp_id?>" rowid="<?=$r->psp_id?>"><?=str_pad($r->psp_numero, 4, '0', STR_PAD_LEFT)?></a>
				</td>
				...
				<td><?=$r->psp_fecha?></td>
				<td><?=$r->tipo_permiso_desc?></td>
				<td><?=$r->psp_fecha_inicio?></td>
				<td><?=$r->psp_fecha_fin?></td>
				<td><?=$r->psp_resolucion?></td>
				<td><?=$r->psp_ruta?></td>
	...
	?>
		...
		<script>
			...
			$('.list-row').click( function(event) {
				window.open('<?=base_url($controller).'/getView?psp_id='?>'+$(event.currentTarget).attr('rowid'), '_self');
			});
		</script>
	</div>
