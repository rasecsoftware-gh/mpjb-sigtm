...
class M_PSP extends CI_Model{
	...
	public function get_tipo_permiso_list () {
		...
		->get('public.tipo_permiso')->result();
	...
	public function get_doc_requisito_list ($doc_id, $tipo_permiso_id) {
		$rows = $this->db
		->select("
			tdr.*,
			dr.doc_requisito_id,
		...
		->where('tdr.tipo_permiso_id', $tipo_permiso_id)
	...
	public function get_vehiculo_row ($id) {
		$row = $this->db->where('public.psp_vehiculo_id', $id)->get('public.psp_vehiculo')->row();
		return $row;
	}

	public function add_vehiculo ($data) {
		$table = 'public.psp_vehiculo';
	...
	public function update_vehiculo ($data) {
		...
		$dr = $this->db->where('psp_vehiculo_id', $data['psp_vehiculo_id'])->get($table)->row();
		$data['syslog'] = sys_session_syslog('modificar', $dr->syslog);

		$this->db->trans_begin();
		$this->db
		->where('psp_vehiculo_id', $data['psp_vehiculo_id'])
		->update($table, $data);
		...
		if ($this->db->trans_status() === FALSE){
        	...
        	$this->db->trans_commit();
        	return $data['psp_vehiculo_id'];
		}
	}

	public function delete_vehiculo ($psp_vehiculo_id) {
		$this->db->trans_begin();
		$this->db->where('psp_vehiculo_id', $psp_vehiculo_id);
		$this->db->delete('public.psp_vehiculo');
