...
class M_GEB extends CI_Model{
	...
	public function get_list ($search_by='general', $search_text='', $size=100, $start=0) {
		$oc_geb_id_list = array(-1);
		if ($search_by == 'oc') {
			$this->db
			->select('d.geb_id')
			->from('alm.geb_det AS d')
			->join('alm.oc_det AS ocd', 'ocd.oc_det_id = d.oc_det_id', 'inner');
			foreach(explode(' ', trim($search_text)) as $term) {
				$this->db->where("ocd.oc_anio||' '||ocd.oc_numero LIKE '%{$term}%'");
			}
			$oc_geb_rows = $this->db->get()->result();
			foreach($oc_geb_rows as $geb) {
				$oc_geb_id_list[] = $geb->geb_id;
			}
		}
		$this->db->start_cache();
		switch ($search_by) {
			...
			break;
			case 'oc':
				$this->db->where_in('geb_id', $oc_geb_id_list);
			break;
		}
	...
	public function import_geb_det_from_oc ($geb_id, $list) {
		...
		foreach ($list as $key => $value) {
			...
			if (is_null($r)) {
				die("ERROR ON: $orden_anio, $orden_numero, $bs_cod, $obs");
			}
